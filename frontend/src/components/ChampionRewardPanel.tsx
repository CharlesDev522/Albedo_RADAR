      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-[10px] text-zinc-500 mt-0.5 max-w-xl">
            SN{subnet} winner (highest incentive) · estimated from metagraph emission ×{" "}
            {champion?.blocks_per_day ?? 7200} blocks/day
          </p>
        </div>
        {champion && (
        ...
        <RewardKpi
          ...
          hint={
            champion?.alpha_price_tao != null
              ? `α × ${champion.alpha_price_tao.toFixed(4)} τ`
              : "needs α price"
          }
        />
        <RewardKpi
        ...
        <RewardKpi
          ...
          value={
            loading && !champion
              ? "…"
              : champion
                ? `${fmtAlpha(champion.emission_per_block_alpha)}`
                : "—"
          }
          accent="text-sky-300"
          hint="metagraph emission"
        />
      </div>
