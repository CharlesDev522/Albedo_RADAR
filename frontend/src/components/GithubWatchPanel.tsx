    ...
    <section className="panel px-3 py-2 mb-3">
      ...
      {targets.length === 0 && !error && (
        <p className="text-[10px] text-amber-300/90 mb-2">
          No GitHub watches configured. Set <code className="mono">GITHUB_REPO_WATCHES</code> in{" "}
          <code className="mono">.env</code> and restart the collector.
        </p>
      )}
      ...
      {alerts.length === 0 ? (
        <p className="text-[10px] text-zinc-600 py-2">
          No new commits recorded yet. First poll seeds the current tip (no alert). Click poll now to
