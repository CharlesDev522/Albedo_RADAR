"""Tests for v5/v6 model commitment parsing."""

from app.chain_reader.commitment_classifier import classify_commitment_raw
from app.chain_reader.commitment_scanner import parse_model_commit, parse_v5, parse_v6
...
def test_parse_v6_commit():
    ...
    assert parse_v5(data, "5Hotkey") is None


def test_parse_v5_commit():
    data = "v5|owner/my-model|sha256:abc123deadbeef"
    parsed = parse_model_commit(data, "5Hotkey")
    assert parsed is not None
    assert parsed["version"] == "v5"
    assert parse_v5(data, "5Hotkey") is not None
