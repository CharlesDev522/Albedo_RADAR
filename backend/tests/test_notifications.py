...
async def test_watcher_reg_fee_skips_already_below_at_bootstrap():
    settings = Settings(
        notifications_enabled=True,
        default_subnet=97,
        notification_reg_fee_thresholds_tao=[0.55],
    )
    dispatcher = NotificationDispatcher(settings)
    dispatcher.enable_resume_mode()
    dispatcher.notify_content = AsyncMock(return_value=True)
    ...
    assert watcher._reg_fee_alerted_tiers == {0.55}
    assert sent == 0
    dispatcher.notify_content.assert_not_awaited()
...
@pytest.mark.asyncio
async def test_watcher_reg_fee_alerts_once_below_threshold():
    settings = Settings(
        notifications_enabled=True,
        default_subnet=97,
        notification_reg_fee_thresholds_tao=[0.55],
    ...
    assert keys == ["reg_fee_low:sn97:tier_0.55"]
...
@pytest.mark.asyncio
async def test_watcher_bootstrap_seeds_eval_queue_validate():
    settings = Settings(notifications_enabled=True, default_subnet=97)
    dispatcher = NotificationDispatcher(settings)
    watcher = NotificationWatcher(dispatcher=dispatcher, settings=settings)

    dashboard = {"reign": {"members": []}, "current_eval": None, "eval_runs": [], "fails": []}
    state = {
        "stages": {
            "hippius_validate": {
                "queued": [
        ...
        "counts": {"hippius_validate": {"queued": 1, "running": 0}},
    }
    with (
        patch("app.notifications.watcher.fetch_dashboard", return_value=dashboard),
        patch("app.notifications.watcher.fetch_state", return_value=state),
        patch(
            "app.notifications.watcher.fetch_subnet_economics",
            return_value={"registration_burn_tao": 1.5},
        ),
    ...
    assert dispatcher.is_seen("eval_queue_entered:sub-seed-1")


@pytest.mark.asyncio
async def test_watcher_emits_eval_queue_entered_on_new_validation_entry():
    settings = Settings(notifications_enabled=True, default_subnet=97)
    dispatcher = NotificationDispatcher(settings)
    dispatcher.enable_resume_mode()
    dispatcher.notify_content = AsyncMock(return_value=True)
    ...
    state = {
        "stages": {
            "hippius_validate": {
                "queued": [
                    {
                        "submission_id": "sub-validate-new",
                        "uid": 12,
                        "hotkey": "hk12",
                        "model_uri": "org/challenger@sha256:new",
                        "state": "SUBMITTED",
                        "updated_at": "2026-06-27T10:15:00+00:00",
                    }
                ],
                "running": [],
            }
    ...
    assert sent >= 1
    kinds = [call.args[1].kind for call in dispatcher.notify_content.await_args_list]
    assert "eval_queue_entered" in kinds
    alert = next(
        c.args[1] for c in dispatcher.notify_content.await_args_list if c.args[1].kind == "eval_queue_entered"
    )
    assert alert.source_key == "eval_queue_entered:sub-validate-new"
    assert alert.detail["state"] == "SUBMITTED"
    assert alert.detail["stage"] == "hippius_validate"
