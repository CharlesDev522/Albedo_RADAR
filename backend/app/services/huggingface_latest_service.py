...
async def fetch_latest_huggingface_repos(
    *,
    ...
    search_created: dict[str, datetime | None] = {}
    ...
    async def _collect_hits(c: httpx.AsyncClient) -> None:
        for query in HF_DISCOVERY_QUERIES:
            ...
            for hit in hits:
                prev = search_created.get(hit.repo)
                search_created[hit.repo] = _merge_created(prev, hit.created_at)
    ...
    async def _hydrate(c: httpx.AsyncClient) -> None:
        for repo, search_created_at in candidates[:fetch_cap]:
            family = infer_albedo_model_family(repo)
            snap = await hf.fetch_model(repo, client=c)
            if snap is None:
                continue
            indexed_at = search_created_at
            models.append(
