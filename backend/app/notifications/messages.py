...
KIND_LABELS: dict[AlertKind, str] = {
    ...
    "eval_queue_entered": "Eval Queue (Validation)",
}
...
_VALIDATE_STATE_LABELS: dict[str, str] = {
...
def _validate_state_label(state: str | None) -> str:
    if not state:
        return "entered validation pipeline"
    return _VALIDATE_STATE_LABELS.get(state, state.replace("_", " ").lower())


def build_eval_queue_entered_alert(
    *,
    netuid: int,
    source_key: str,
    detail: dict[str, Any],
    ...
    uid = detail.get("uid")
    uid_s = f"uid {uid} " if uid is not None else ""
    state_label = _validate_state_label(detail.get("state"))
    return AlertContent(
        kind="eval_queue_entered",
        title=f"[eval_queue_entered] {label}",
        message=f"SN{netuid} {uid_s}{state_label}",
        source_key=source_key,
        detail=detail,
        subnet=netuid,
    )
